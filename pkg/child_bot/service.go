...
func (s *service) Serve(ctx context.Context) error {
	...
	mux.HandleFunc(tokenPrefixPath, func(w http.ResponseWriter, r *http.Request) {
...
func (s *service) handleOwnerStart(
	...
	err = s.reply(api, upd, "Какой текст бот должен отвечать "+
		"когда нажимают кнопку «Начать»? Например: _Привет, слушаю вас_")
	if err != nil {
		return fmt.Errorf("s.reply: %w", err)
	}
...
- Стратегия. Если указано *1* — бот сверяет и применяет правила только на первое сообщение, далее не вмешивается в вашу переписку с отправителем. Если указано *2* — бот применяет правила автоответа и на первое сообщение отправителя, и на дальнейшие;
...
func (s *service) handlePeer(ctx context.Context, api *tgbotapi.BotAPI, upd update, bot Bot) error {
	if bot.Mode == OnlyFirst && upd.Message.MessageID != 1 {
		return nil
	}
...
Вы можете _Ответить_ на это сообщение текстом, чтобы ответить отправителю, или _Ответить_ *%s* чтобы забанить его`,
